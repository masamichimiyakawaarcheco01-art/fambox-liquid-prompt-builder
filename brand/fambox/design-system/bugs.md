...
### DOCTRINE-010: B向けHeroの“特大ジャンプ”は構造で出す（横組みパラメータ調整は ~57% で頭打ち）
- **ルール**: B向け(法人)チラシ表 Hero の特大ジャンプ率は、**横組み見出しのフォント拡大では A4 幅(595)制約で ~30pt が上限**（36pt で 3 行折返し＋stat pill 衝突）。ゴールド(`reference/human-B-front`)水準の躍動は **縦書き装飾見出し（YuMincho 等）＋切り抜き選手コラージュへの restructure** で初めて到達する
- **含意（採点で切り分ける）**: `references/quality-rubric.md`（9軸/合格80%）で採点したとき、**写真主導(18%)・ジャンプ率(16%)の不足はパラメータ調整では動かず、型(template)の構造変更が必要**。フォント拡大・トラッキングだけの改修は **~57% が天井**（実測: B-front-codex 121:8 = 54% → 改修後 57%、+3pt のみ）
- **運用**: 70%+ を狙うサイクルは最初から restructure（縦書き特大＋写真起点 Hero）を選ぶ。「フォント拡大で頑張る」罠に入らない。採点→ギャップが**「構造由来」か「調整由来」か**を判定してから着手する
- **Why**: 2026-06-09 品質クリティーク・ループ bootstrap。Codex 生成の B-front-codex を rubric 採点(54%)→ジャンプ率改修→再採点(57%)し、**パラメータ調整の低い天井を数値で実証**。v3-comparison/v4-how-to-reach-70 の「型を作り替えよ」結論と一致
- **出自**: 品質クリティーク・ループ v0（`docs/okr/fambox-flywheel/quality-loop/CRITIQUE-AGENT.md`）、`references/quality-rubric.md`、fambox-flyer-builder スキル

### DOCTRINE-011: 役割ドリブン設計 ＋ AIの自己採点は過大（完成判定は人間/別モデル）
- **設計原則（宮川さん＝デザイナー校正 2026-06-09）**:
  - **何を見せる狙いかを先に決め、情報・写真・余白をその狙いに合わせて設計する**。Hero=誘目（大見出し・余白・写真でOK）／説明エリア(ISSUES/SOLUTION等)=理解（**情報量＋背景画像等のビジュアルで密度**を持たせる。テキストだけは弱い＝スカスカ厳禁）
  - **タイポ**: ジャンプ率は「大きいほど良い」ではない。**写真エリアと両立する適正サイズ**（写真主役 Hero の見出しは ~32px が目安、44pxは過大）。**行間＝見出し150–170%／本文≥150%**（詰めると読む気を削ぐ）
  - **余白は情報設計**: 役割が同じ＝小余白、役割が異なる（タイトル↔詳細）＝大余白。**タイトル直下 11px 等の詰まりは失点**
  - **浮遊コンポーネントの重なりは意図的に**: stat pill 等は写真エリアと**約50%（例 60px 中 30px）重ねる**のがバランス。中途半端な重なり位置はNG
- **メタ学習（最重要）**: **AIの自己採点は甘い（過大評価する）**。restructure 後の v2 を AI が 72% と自己採点 → 宮川さん実査で **~60%**。**完成判定は人間 or 別モデルの独立採点を必須**とし、AI単独の「できた」を信用しない（quality-rubric v1・クロス採点と整合）
- **Why**: 2026-06-09 restructure サイクルで AI 72%自己評価の v2(130:8) を宮川さんが実査。Hero見出し過大(44→32px)・行間詰まり(116%→150%+)・余白の役割不足(11px)・各エリア情報量不足(スカスカ)・stat pill 重なり中途半端 を具体指摘。rubric を v1 に校正
- **出自**: 宮川さん v2 実査校正、`references/quality-rubric.md` v1、fambox-flyer-builder スキル

### DOCTRINE-012: ゴールド級チラシの設計思想（人間 v3 × AI v2 の diff）
- **核**: **役割ドリブン × Show-not-Tell × 情報リッチ**。各セクションの「説得の役割」に比例して**面積・写真・コピー・装飾を投資**する。薄い骨格を均等配分しない（これが AI の頭打ちの正体）
- **面積配分**: 誘目(Hero)は紙面 **~50%(421/842px)** まで大胆投資。信頼(監修者)にも十分面積。均等割りにしない
- **行間（標準値を引き上げ）**: 本文 **160–170%**、テーマ見出し **180%** を標準に。150% は下限であって正解ではない（v3実測：VALUE 170%／bio 160%／テーマ 180%）
- **Show-not-Tell**: **全セクションに写真/ビジュアル**を入れる。テキスト箇条書きだけのセクションを作らない。IA は**課題先行でなく価値・アスピレーション先行**（v3 は「課題」を捨て「VALUES」で見せた）
- **言葉選び**: **具体×情緒×証拠**。例「ただの食事じゃない、その1食」「完全装備」「20年超えの現場知見」＋実証「通算148勝」「2000年から」。汎用コピー（「土台に」「大幅軽減」）は弱い
- **レイヤー構造**: 5–6層の意図的 z 深度＝背景グラデ → 写真コラージュ(画面外へブリード) → アクセント円 → 円マスク被写体 → 縦書き特大見出し → スクリプト重ね。浅い2–3層は平板
- **色使い**: フラット単色を避ける。**グラデーション＋半透明(白10%のガラスカード)＋ニアブラック `#1B1D1A`(信頼/監修)＋Drive Orange `#FB5215`(エネルギー)＋アクセント鮮橙 `#EC3F00`**。役割で色分け
- **装飾語彙（FAM BOX 印刷の標準パーツ）**: Thank You! 手書きスクリプト／**キャラクターライン（カード隅の 40×1px ブラケット）**／円形フォトマスク／橙アクセント円／**ロゴロックアップ（素テキスト不可）**／料理写真インセット
- **縦書き特大見出し（DOCTRINE-010 の実装解）**: Hiragino **Demibold 32px**・文字間スペース・縦長テキストボックス。横組みの幅制約を回避して特大ジャンプを実現
- **フォント**: ブランドは **Hiragino W4/W5 ＋ Demibold**（Mac で適用＝真の人間30%）。構築時 Noto Sans JP は仮置き
- **Why**: 2026-06-09 宮川さんの B-front-codex-v3(126:8、ゴールド級 ~85–90%) と AI restructure 版 v2(130:8、~61%) の diff。差は技量でなく**設計思想**。AI は「役割比例の投資」「Show」「情報密度」が欠落しがち
- **出自**: v3 実査 diff（miyaFB式）、`references/quality-rubric.md` v1、fambox-flyer-builder スキル
...
| DOCTRINE | 12 | Typography 1 / Anti-pattern 1 / 資料フォント 1 / Specificity 1 / Schema 型 1 / 印刷物Typo 1 / 色ゾーニング 1 / 3テイストモード 1 / ブロッキング+フルブリード+レイヤー 1 / B向けHero特大ジャンプは構造で 1 / 役割ドリブン設計+AI自己採点は過大 1 / **ゴールド級設計思想(v3 diff) 1** |
...
| **合計** | **39** | DOCTRINE-012（ゴールド級設計思想・v3 diff）は 2026-06-09 宮川さん B-front-codex-v3 実査 diff から追加 |
...
- v0.7 (2026-06-09): **宮川さん B-front-codex-v3（ゴールド級）と AI v2 の diff から DOCTRINE-012 追加（計39）**。差は技量でなく設計思想＝役割ドリブン×Show-not-Tell×情報リッチ。面積を役割に比例投資（Hero~50%）／行間160–180%標準／全セクションに写真／価値先行IA／具体×情緒×証拠コピー／5–6層z深度＋ブリード／グラデ+半透明+ニアブラック+アクセント橙／装飾語彙(Thank Youスクリプト・キャラクターライン・円マスク・ロゴロックアップ)／縦書き特大はHiragino Demibold 32px。miyaFB式 diff 学習。
- v0.6 (2026-06-09): **宮川さん v2 実査校正から DOCTRINE-011 追加（計38）＋ quality-rubric v1 校正**。AI自己採点72%が人間実査~60%＝過大評価と判明 → 完成判定は人間/別モデル必須。設計原則を明文化：役割ドリブン（狙い→情報/写真/余白）、ジャンプ率は写真両立サイズ(~32px)＋行間≥150%、余白は役割ベース（タイトル直下詰まりNG）、各エリアに十分な情報＋ビジュアル（スカスカ厳禁）、浮遊チップは写真と約50%重ね。rubric に新軸「エリア目的適合（情報量）」追加・甘い軸を是正。
- v0.5 (2026-06-09): **品質クリティーク・ループ bootstrap から DOCTRINE-010 追加（計37）**。`references/quality-rubric.md`（9軸/合格80%/ゴールド4枚）＋ `quality-loop/CRITIQUE-AGENT.md` を新設し、Codex 生成 B-front-codex(121:8) を採点(54%)→ジャンプ率改修→再採点(57%)。**フォント拡大等のパラメータ調整は ~57% が天井で、写真主導(18%)・特大ジャンプ(16%)は型の restructure（縦書き特大＋写真起点Hero）でしか動かない**ことを数値実証。「採点→構造由来/調整由来の切り分け→恒久ルール化」で愚直FBを複利ループ化（Flywheel Learn 層の機械化）。
