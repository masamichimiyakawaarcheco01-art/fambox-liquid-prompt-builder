<!--
Codex 用ルーター（LIVE）。このフォルダ Claude_1/ で Codex を起動すると自動読込される。
Claude Code の「スキル自動発火」を Codex 側で再現するため、どの入力で何を読むかを明示する。
元の草案：docs/okr/fambox-flywheel/codex/AGENTS.flywheel-router.md ／ 移行手順：docs/okr/fambox-flywheel/CODEX-HANDOFF.md
Claude Code 利用時の指示は同フォルダ CLAUDE.md（本ファイルと共存）。
-->
